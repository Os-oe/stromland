...
with sync_playwright() as p:
    ...
    # Intro-Tagesfahrt läuft beim frischen Load auf der Live-Domain
    ok(page.evaluate('window.__stromland.introActive()'), 'Intro-Tagesfahrt läuft live')
    t0 = page.text_content('#hud-title')
    page.wait_for_timeout(2500)
    t1 = page.text_content('#hud-title')
    ok(t0 != t1, f'Intro live: HUD-Uhr wandert („{t0}" → „{t1}")')
    # Klick bricht zum Live-Modus ab — danach müssen HUD-Werte == Live-API sein
    page.mouse.click(640, 400)
    page.wait_for_timeout(1500)
    ok(not page.evaluate('window.__stromland.introActive()'), 'Klick bricht Intro live ab')
